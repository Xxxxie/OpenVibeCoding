...
function promptInput(prompt: string, hidden = false): Promise<string> {
  return new Promise((resolve) => {
    ...
    if (hidden) {
      // For password input, hide the characters
 ...
 */
async function listNamespaces(client: any): Promise<NamespaceInfo[]> {
  try {
    // Use DescribeRepositoryOwnerPersonal to get namespaces
    const result = await client.DescribeRepositoryOwnerPersonal({
      Limit: 100,
    })
    if (result?.RepoInfo?.length > 0) {
      // Extract unique namespaces
      const namespaceSet = new Set<string>()
      const namespaces: NamespaceInfo[] = []
      for (const repo of result.RepoInfo) {
        const ns = repo.Namespace || repo.RepoNamespace
        if (ns && !namespaceSet.has(ns)) {
          namespaceSet.add(ns)
          namespaces.push({ Namespace: ns })
        }
      }
      return namespaces
    }
    ...
    log(`Failed to list namespaces: ${error.message}`, 'warn')
    return []
  }
}
 ...
 */
async function createNamespaceWithSuffix(client: any, prefix: string, maxRetries = 3): Promise<string | null> {
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      ...
      if (error.code === 'ResourceInUse' || error.message?.includes('already')) {
        log(`Namespace '${namespace}' already exists, trying another suffix...`, 'warn')
        continue
      }
      log(`Failed to create namespace: ${error.message}`, 'error')
      return null
    }
  }
...
async function validateAndPrepareEnv(config: SetupConfig): Promise<boolean> {
  ...
  if (!config.secretId) {
    const envValue = env['TCB_SECRET_ID'] || env['TENCENTCLOUD_SECRET_ID']
    if (envValue) {
      config.secretId = envValue
    }
  }

  if (!config.secretKey) {
    const envValue = env['TCB_SECRET_KEY'] || env['TENCENTCLOUD_SECRET_KEY']
    if (envValue) {
      config.secretKey = envValue
    }
  }

  if (!config.accountId) {
    const envValue = env['TENCENTCLOUD_ACCOUNT_ID']
    if (envValue) {
      config.accountId = envValue
    }
  }
  ...
  // Step 3: Try cloudbase-cli credentials
